...
static uint8_t calc_prescale(uint16_t frequency_hz) {
    ...
    return (uint8_t)prescaleval;
}

static esp_err_t pca9685_apply_frequency(uint8_t address, uint16_t frequency_hz) {
    uint8_t prescale = calc_prescale(frequency_hz);

    uint8_t mode1 = 0x10;  // sleep
    ESP_RETURN_ON_ERROR(pca9685_write(address, PCA9685_MODE1, &mode1, 1), TAG, "sleep");
    ESP_RETURN_ON_ERROR(pca9685_write(address, PCA9685_PRESCALE, &prescale, 1), TAG, "prescale");
    vTaskDelay(pdMS_TO_TICKS(5));
    mode1 = 0x20;  // auto increment, normal
    ESP_RETURN_ON_ERROR(pca9685_write(address, PCA9685_MODE1, &mode1, 1), TAG, "wake");
    vTaskDelay(pdMS_TO_TICKS(5));
...
esp_err_t pca9685_init(uint8_t address, uint16_t frequency_hz) {
    ...
    uint8_t mode2 = 0x04;  // totem pole
    ESP_RETURN_ON_ERROR(pca9685_write(address, PCA9685_MODE2, &mode2, 1), TAG, "mode2");
    return pca9685_apply_frequency(address, frequency_hz);
}
...
esp_err_t pca9685_set_frequency(uint8_t address, uint16_t frequency_hz) {
    return pca9685_apply_frequency(address, frequency_hz);
}
