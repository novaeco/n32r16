...
static esp_err_t websocket_handler(httpd_req_t *req) {
    ...
    if (ret == ESP_OK && s_rx_cb != NULL) {
        bool is_text = (frame.type == HTTPD_WS_TYPE_TEXT);
        s_rx_cb(frame.payload, frame.len, is_text);
    }
    free(frame.payload);
    return ret;
}
...
esp_err_t ws_server_send(const uint8_t *data, size_t len, bool is_text) {
    ...
    httpd_ws_frame_t frame = {
        ...
        .len = len,
        .type = is_text ? HTTPD_WS_TYPE_TEXT : HTTPD_WS_TYPE_BINARY,
    };
